...
class TestAuthResult:
    ...
    def test_auth_result_creation(self):
        ...
        result = AuthResult(
            user=user, auth_type=AuthType.SYSTEM, metadata={"api_key_id": 1}
        )
    ...
    def test_auth_result_no_metadata(self):
        ...
        user = UserFactory.create_mock_user()
        result = AuthResult(user=user, auth_type=AuthType.CLERK)
...
class TestClerkAuthStrategy:
    ...
    async def test_authenticate_success(self):
        ...
        mock_db_user = UserFactory.create_mock_user(
            auth_type=AuthType.CLERK, clerk_user_id="clerk_user_123"
        )
        mock_db.query.return_value.filter.return_value.first.return_value = mock_db_user

        with patch.object(
            strategy.clerk_client,
            "authenticate_request",
            return_value=mock_request_state,
        ):
            with patch.object(
                strategy.clerk_client.users, "get", return_value=mock_user_response
            ):
    ...
    async def test_authenticate_inactive_user(self):
        ...
        mock_db_user = UserFactory.create_mock_user(
            auth_type=AuthType.CLERK, clerk_user_id="clerk_user_123", is_active=False
        )
        mock_db.query.return_value.filter.return_value.first.return_value = mock_db_user
...
class TestAuthStrategyManager:
    ...
    async def test_authenticate_api_key_first(self):
        ...
        with patch.object(
            manager.strategies[AuthType.SYSTEM],
            "authenticate",
    ...
    async def test_authenticate_clerk_fallback(self):
        ...
        with patch.object(
            manager.strategies[AuthType.CLERK], "authenticate", return_value=mock_result
        ):
            result = await manager.authenticate(mock_request, mock_db)
    ...
    async def test_authenticate_both_headers_prefers_api_key(self):
        ...
        with patch.object(
            ...
            with patch.object(
                manager.strategies[AuthType.CLERK], "authenticate"
            ) as mock_clerk:
                result = await manager.authenticate(mock_request, mock_db)
