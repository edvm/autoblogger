...
def test_system_user_registration():
    ...
    response = requests.post(
        f"{API_BASE_URL}/api/v1/auth/register", json=registration_data
    )
...
def test_blog_generation(api_key: str):
    ...
    blog_data = {"topic": "Test Blog Post", "search_depth": "basic", "max_results": 3}

    response = requests.post(
        f"{API_BASE_URL}/api/v1/apps/blogger/generate", headers=headers, json=blog_data
    )
...
def test_api_key_management(api_key: str):
    ...
    # Create a new key
    new_key_data = {"name": "Test API Key", "expires_at": None}

    response = requests.post(
        f"{API_BASE_URL}/api/v1/auth/keys", headers=headers, json=new_key_data
    )
...
def main():
    ...
    print(
        f"   Authenticated as: {user_info.get('username')} ({user_info.get('email')})"
    )
    print()
